...
public class ChunkGenerator : MonoBehaviour
{
    ...
    [Tooltip("How many chunks to load around the player (16 = 33x33 grid)")]
    ...
    void Start()
    {
        if (player == null)
        {
            ...
            if (player == null)
            {
                Debug.LogError("ChunkGenerator: No player found! Make sure the player has the 'Player' tag.");
                return;
            }
        }
        ...
        if (showDebugInfo)
        {
            Debug.Log($"ChunkGenerator initialized - Chunk Size: {chunkSize}, Render Distance: {renderDistance}");
        }
    }
    ...
    void SetupPersistenceManager()
    {
        if (enablePersistence)
        {
            ...
            persistenceManager.SetChunkGenerator(this);
        }
    }
        ...
        for (int x = -renderDistance; x <= renderDistance; x++)
        {
            for (int z = -renderDistance; z <= renderDistance; z++)
            {
                Vector2Int chunkPos = new Vector2Int(
                    playerChunk.x + x,
                    playerChunk.y + z  // Changed from .z to .y
                );
    ...
    void OnDrawGizmosSelected()
    {
        if (player != null)
        {
            ...
            Vector3 chunkCenter = new Vector3(
                ...
                0,
                playerChunk.y * chunkSize + chunkSize / 2f  // Changed from .z to .y
            );
